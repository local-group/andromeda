...
use native_tls::{self, TlsConnector, TlsAcceptor, Pkcs12};
...
cfg_if! {
    if #[cfg(feature = "rustls")] {
        ...
        pub fn contexts() -> (TlsAcceptor, TlsConnector) {
        ...
        pub fn contexts2() -> (TlsAcceptor, TlsConnector, TlsConnector) {
            let keys = openssl_keys();
            ...
            let mut client = t!(TlsConnector::builder());
            let mut client_copy = t!(TlsConnector::builder());
            t!(client.builder_mut()
               .builder_mut()
               .set_ca_file(&path));
            t!(client_copy.builder_mut()
               .builder_mut()
               .set_ca_file(&path));

            (t!(srv.build()), t!(client.build()), t!(client_copy.build()))
        }
