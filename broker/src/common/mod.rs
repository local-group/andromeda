          ...
          Key: Eq + Hash + Clone,
{
    ...
    fn insert_topic(&mut self, user_id: UserId, topic: &Topic, key: &Key, item: P::Item) -> bool {
    ...
    fn remove_topic(&mut self, user_id: UserId, topic: &Topic, key: &Key) {
    ...
    fn remove_all_topics(&mut self, user_id: UserId, key: &Key) -> usize {
        if let Some(ref mut topics) = self.topics.get_mut(&user_id) {
    ...
    fn search(&self, user_id: UserId, topic_name: &TopicName) -> P {
        let mut peers = P::new();
